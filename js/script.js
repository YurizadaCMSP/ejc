...
const CONFIG = {
    ...
    // Data alvo: 02 de setembro de 2026, 00:00, Horário de Brasília (UTC-3)
    // Pode ser sobrescrito pelo AGORASIM_CONFIG.targetDate
...
function cacheElements() {
    ...
    elements.openLivraoWrapper = document.getElementById("open-livrao-wrapper");
    elements.openLivraoBtn = document.getElementById("open-livrao-btn");
    elements.openLivraoText = document.getElementById("open-livrao-text");
    ...
    // Popup de sigilo
    elements.sigiloPopup = document.getElementById("sigilo-popup");
    elements.sigiloTitle = document.getElementById("sigilo-title");
    elements.sigiloMessage = document.getElementById("sigilo-message");
    elements.sigiloCloseBtn = document.getElementById("sigilo-close-btn");
}
...
function setupConfig() {
    // Usa data do AGORASIM_CONFIG se disponível
    if (window.AGORASIM_CONFIG && window.AGORASIM_CONFIG.targetDate) {
        CONFIG.targetDate = window.AGORASIM_CONFIG.targetDate;
    }
    // Usa logo do AGORASIM_CONFIG se disponível
    if (window.AGORASIM_CONFIG && window.AGORASIM_CONFIG.logo) {
        CONFIG.logo = window.AGORASIM_CONFIG.logo;
    }
    // Usa texto do botão do AGORASIM_CONFIG se disponível
    if (window.AGORASIM_CONFIG && window.AGORASIM_CONFIG.index) {
        if (window.AGORASIM_CONFIG.index.buttonText && elements.openLivraoText) {
            elements.openLivraoText.textContent = window.AGORASIM_CONFIG.index.buttonText;
        }
        if (window.AGORASIM_CONFIG.index.redirectUrl && elements.openLivraoBtn) {
            elements.openLivraoBtn.href = window.AGORASIM_CONFIG.index.redirectUrl;
        }
    }
...
function finishCountdown() {
    ...
    // Mostra o botão "Abrir Livrão"
    if (elements.openLivraoWrapper) {
        elements.openLivraoWrapper.classList.remove("hidden");
        void elements.openLivraoWrapper.offsetWidth;
        elements.openLivraoWrapper.classList.add("visible");
    }
}

// ==================== POPUP DE SIGILO ====================
function setupSigiloPopup() {
    if (!window.AGORASIM_CONFIG || !window.AGORASIM_CONFIG.sigilo) return;
    if (!elements.sigiloPopup) return;

    const sigilo = window.AGORASIM_CONFIG.sigilo;

    // Preenche conteúdo
    if (elements.sigiloTitle) elements.sigiloTitle.textContent = sigilo.title || "Sigilo no EJC";
    if (elements.sigiloMessage) elements.sigiloMessage.innerHTML = sigilo.message || "";

    // Decide se mostra o popup
    const shouldShow = sigilo.alwaysShow || !sessionStorage.getItem("ejc_sigilo_dismissed");

    if (shouldShow) {
        // Mostra após a animação de entrada do site
        setTimeout(() => {
            showSigiloPopup();
        }, 1500);
    }
    ...
    // Botão de fechar
    if (elements.sigiloCloseBtn) {
        elements.sigiloCloseBtn.addEventListener("click", () => {
    ...
    document.addEventListener("keydown", (e) => {
        if (e.key === "Escape" && elements.sigiloPopup && !elements.sigiloPopup.classList.contains("hidden")) {
            sessionStorage.setItem("ejc_sigilo_dismissed", "true");
            hideSigiloPopup();
        }
    });
...
function showSigiloPopup() {
    if (!elements.sigiloPopup) return;
    elements.sigiloPopup.classList.remove("hidden");
    void elements.sigiloPopup.offsetWidth;
    elements.sigiloPopup.classList.add("visible");
    document.body.style.overflow = "hidden";
    if (elements.sigiloCloseBtn) elements.sigiloCloseBtn.focus();
}

function hideSigiloPopup() {
    if (!elements.sigiloPopup) return;
    elements.sigiloPopup.classList.remove("visible");
    setTimeout(() => {
        elements.sigiloPopup.classList.add("hidden");
